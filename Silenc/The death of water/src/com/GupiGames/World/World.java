...
import com.GupiGames.Entities.soul;
import com.GupiGames.main.Game;

public class World extends Rectangle{
	
	private static final long serialVersionUID = 2709276140853894562L;
		...
		try {
			...
			for(int axisX = 0; axisX < WIDTH; axisX++) {
				...
				for(int axisY = 0; axisY < HEIGHT; axisY++) {
					...
					case(0xFF5b6ee1):
						Game.entities.add(new soul(WallTile.Blank, axisX*16, axisY*16, 16, 16, "soul"));
						tiles[axisX + (axisY * map.getWidth())] = new Tile(WallTile.Blank, axisX*16, axisY*16, 16, 16);;
					break;
					case(0xFF639bff):
						Game.entities.add(new soul(WallTile.Blank, axisX*16, axisY*16, 16, 16, "bigSoul"));
						tiles[axisX + (axisY * map.getWidth())] = new Tile(WallTile.Blank, axisX*16, axisY*16, 16, 16);;
					break;
	...
	public static void reset() {
		Game.pressurePlates.clear();
		Game.doors.clear();
		Game.entities.clear();
		Game.souls.clear();
		
		Game.pressurePlates = new ArrayList<Tile>();
		Game.doors = new ArrayList<Tile>();
		Game.player = new Player(Game.playerSheet.getSprite(0, 0, 16, 16), 16, 16, 16, 16);
		Game.entities = new ArrayList<Entity>();
		Game.souls = new ArrayList<Entity>();
	}
	...
	public static boolean isFall(Entity entity, int plusY) {
		...
		for(int axisX = xStart; axisX < xFinal; axisX++) {
			for(int axisY = yStart; axisY < yFinal; axisY++) {
				...
				if(tileCollider.intersects(entityCollider) && (tile instanceof FloorTile || tile instanceof WallTile)) {
					return false;
				}
